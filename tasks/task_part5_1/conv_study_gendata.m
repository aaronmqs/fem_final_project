clear; clc; close all;

pmax = 3;
kmax = 4;
pltit = false;

%% Simplex
etype = 'simp';
fprintf(etype)
Es = zeros(kmax, pmax); hs = Es; ocr = zeros(pmax, 1);
for porder = 1:pmax
    for k = 1:kmax
        nel = 2^k * [2 2];

        % Compute error
        [~, Es(k, porder)] = solve_linelptc_sclr_disk0(etype, nel, porder, pltit);

        % Compute element size
        msh = create_mesh_hsphere(etype, [0; 0], 1, nel, porder);
        v = measure_domain(msh, porder, msh.ndim, etype);
        hs(k, porder) = sqrt(v / size(msh.e2vcg, 2));
    end
    ocr(porder) = log(Es(1, porder) / Es(end, porder)) / log(hs(1, porder) / hs(end, porder));
    loglog(hs, Es(:, porder), 'Marker','square'); grid on; hold on;
    fprintf("\n p = " + num2str(porder)+ ", Opt. conv. rate = " + num2str(ocr(porder)) + "\n\n")
end

legend("p1", "p2", "p3")

% save("Es", "Es")
% save("hs", "hs")

%% Hypercube
figure
etype = 'hcube';
Eh = zeros(kmax, pmax); hh = Eh; ocr = zeros(pmax, 1);
for porder = 1:pmax
    for k = 1:kmax
        nel = 2^k * [2 2];

        % Compute error
        [~, Eh(k, porder)] = solve_linelptc_sclr_disk0(etype, nel, porder, pltit);

        % Compute element size
        msh = create_mesh_hsphere(etype, [0; 0], 1, nel, porder);
        v = measure_domain(msh, porder, msh.ndim, etype);
        hh(k, porder) = sqrt(v / size(msh.e2vcg, 2));
    end
    ocr(porder) = log(Eh(1, porder) / Eh(end, porder)) / log(hh(1, porder) / hh(end, porder));
    loglog(hh, Eh(:, porder), 'Marker','square'); grid on; hold on;
    fprintf("p = " + num2str(porder)+ ", Opt. conv. rate = " + num2str(ocr(porder)) + "\n\n")
end

legend("p1", "p2", "p3")

% save("Eh", "Eh")
% save("hh", "hh")

% simp
%  p = 1, Opt. conv. rate = 1.8194
% 
%  p = 2, Opt. conv. rate = 3.6555
% 
%  p = 3, Opt. conv. rate = 4.004
% 
% hcube
% p = 1, Opt. conv. rate = 2.2011
% 
% p = 2, Opt. conv. rate = 3.5855
% 
% p = 3, Opt. conv. rate = 3.8962